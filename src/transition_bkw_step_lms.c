...
int transition_bkw_step_lms(const char *srcFolderName, const char *dstFolderName, bkwStepParameters *srcBkwStepPar, bkwStepParameters *dstBkwStepPar, u64 *numSamplesStored, time_t start)
{
    ...
    while (numReadCategories && (storageWriterCurrentLoadPercentage(&sw) < EARLY_ABORT_LOAD_LIMIT_PERCENTAGE))
    {
        ...
        case LF1:
            ...
            default:
                timeStamp(start);
                printf("*** transition_bkw_step_lms: Unexpected number of categories\n");
            }
            break;
        case LF2:
            switch (numReadCategories)
