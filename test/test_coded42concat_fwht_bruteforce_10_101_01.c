...
//  u64 totalNumInitialSamples = 1000000; /* 1 million */
    ...
    sprintf(outputfolder, "%s/test_coded42concat_bkw_fwht_bruteforce_10_101_01", LOCAL_SIMULATION_DIRECTORY_PATH_PREFIX_A);
    ...
    /* Set steps: codedBKW(2,1) concatenated twice, 4 positions */
    ...
    timeStamp(start);
    printf("Last reduction step %02d -> %02d, %s reduction at positions %d to %d (destination sorting using positions %d to %d)\n", i, i+1, sortingAsString(bkwStepPar[i].sorting), bkwStepPar[i].startIndex, bkwStepPar[i].startIndex + bkwStepPar[i].numPositions - 1, bkwStepPar[i].startIndex, bkwStepPar[i].startIndex + bkwStepPar[i].numPositions - 1);
    sprintf(srcFolderName, "%s/step_%02d", outputfolder, i);
